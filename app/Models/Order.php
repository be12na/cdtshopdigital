...
class Order extends Model
{
    ...
    public function update_stock($returned = false)
    {
        foreach ($this->items as $item) {
            $parentProduct = $item->product_id
               ? Product::where('id', $item->product_id)->lockForUpdate()->first()
               : null;

            if ($parentProduct && $parentProduct->is_unlimited_stock) {
                continue;
            }
            ...
            if ($parentProduct) {
                if ($parentProduct->sku && $parentProduct->sku === $item->sku) {
                    $target = $parentProduct;
                } else {
                    $target = ProductVarian::where('product_id', $parentProduct->id)
                        ->where('sku', $item->sku)
                        ->lockForUpdate()
                        ->first();
                    if (! $target) {
            ...
            if (! $target) {
                $target = Product::where('sku', $item->sku)->lockForUpdate()->first();
                if ($target && $target->is_unlimited_stock) {
                    continue;
                }
                if (! $target) {
                    $target = ProductVarian::where('sku', $item->sku)->lockForUpdate()->first();
                    if ($target && $target->product && $target->product->is_unlimited_stock) {
                        continue;
                    }
                }
            }
            ...
            if ($returned) {
                ...
                $target->stock = max(0, $target->stock - $item->quantity);
            }

            $target->save();

            if ($parentProduct) {
                Cache::forget('product_'.$parentProduct->slug);
                Cache::forget('product_'.$parentProduct->id);
            } elseif ($target instanceof Product) {
                Cache::forget('product_'.$target->slug);
                Cache::forget('product_'.$target->id);
            } elseif ($target instanceof ProductVarian && $target->product) {
                Cache::forget('product_'.$target->product->slug);
                Cache::forget('product_'.$target->product->id);
            }
        }
    }
}
