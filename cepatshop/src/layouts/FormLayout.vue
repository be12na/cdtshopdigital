<template>
   <q-layout view="hHh lpR fFf" class="bg-light overflow-x-hidden main-layout"
      ...
      <q-header :class="{ 'bg-white text-grey-8 box-shadow q-pt-sm': isActiveTheme != 'elegant', 'bg-brand text-white': isActiveTheme == 'elegant' }">
         <q-toolbar>
         ...
         <q-tabs v-if="is_mode_desktop" inline-label indicator-color="primary">
            ...
            <q-route-tab icon="source" label="Produk" :to="{ name: 'ProductIndex' }" exact />
      ...
      <q-footer class="bg-white text-primary footer-tab box-shadow-top" v-if="!is_mode_desktop">
