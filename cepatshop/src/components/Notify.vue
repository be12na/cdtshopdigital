<template>
   <div data-nosnippet="true" class="notif-content" :class="{ 'show': isShowen }" v-if="orderItem">
      <q-list>
         <q-item>
            ...
            <q-item-section>
               ...
               <div class="text-green-7 q-pa-xs text-xs" v-if="orderItem.created">{{ orderItem.created }}</div>
            </q-item-section>
   ...
   methods: {
      ...
      getRandomOrder() {
         Api.get('order-random').then(res => {
            if (res.status == 200) {
               this.$store.commit('order/SET_NOTIFY_ORDER_ITEMS', res.data.data)
               setTimeout(() => {
